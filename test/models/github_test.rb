...
class GithubTest < MiniTest::Unit::TestCase
  ...
  def expect( _start_time, _end_time = _start_time + 600, _sleep_time = 10 )
    ...
    _expecteds = _start_expecteds  + _task_1_expecteds + _task_2_expecteds + _end_expecteds
    ...
    _task_1_i, _task_2_i = 0, 0
    _logs.each_with_index do |_log, _log_i|
      if _log_i < _start_expecteds.length or
         _log_i > ( _start_expecteds + _task_1_expecteds + _task_2_expecteds ).length
        ...
        if _task_1_expecteds[ :model_id ] == _log[ :model_id ]
          assert_equal _task_1_expected[ :message ], _log[ :message ]
          _task_1_i += 1
        else
          assert_equal _task_2_expected[ :message ], _log[ :message ]
          _task_2_i += 1
        end
      end
    end
