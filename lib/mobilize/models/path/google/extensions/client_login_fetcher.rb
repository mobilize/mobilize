module GoogleDrive
  class ClientLoginFetcher
    ...
    def fatal_response(identifier,response)
      ...
      Mobilize::Logger.error "#{identifier} failed with " +
                             "#{response.body[0..response_log_length]}(...);" +
                             "no retry possible"
    end
    ...
    def exponential_retry_response(identifier,response,current_retries)
      ...
      Mobilize::Logger.info "#{identifier} failed with " +
                            "#{response.body[0..response_log_length]}(...);" +
                            " retry #{current_retries.to_s} of #{total_retries.to_s} " +
                            "in #{exponential_sleep_time.to_s}"
    ...
    def success_response(identifier,response,current_retries)
      response_log_length   = Mobilize.config.google.api.response_log_length
      Mobilize::Logger.info "#{identifier} returned " +
                            "#{response.body[0..response_log_length]}(...)" +
    ...
    def exhausted_response(identifier,response,current_retries)
      response_log_length    = Mobilize.config.google.api.response_log_length
      Mobilize::Logger.error "Unable to #{identifier} with " +
                             "#{response.body[0..response_log_length]}(...) " +
                             "with #{current_retries.to_s}"
    end
  end
end
